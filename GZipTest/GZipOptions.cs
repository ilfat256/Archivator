...
namespace GZipTest
{
    public class GZipOptions
    {
        ...
        {
            FileStreamBufferSize = 8 * 1024;
            ThreadCount = Environment.ProcessorCount;
            ulong installedRamMemoryBytes;
#warning check return?
            GetPhysicallyInstalledSystemMemory(out installedRamMemoryBytes);
#warning why 25?
            var tasksByThread = 25; //должны занимать, не больше памяти деленной на 2
            TaskCount = tasksByThread * ThreadCount;
            var readingBlockSize = installedRamMemoryBytes / 25 * 8;
            if (readingBlockSize > int.MaxValue)
            {
                readingBlockSize = int.MaxValue;
            }
            ReadingBlockSize = (int)readingBlockSize;
        }
